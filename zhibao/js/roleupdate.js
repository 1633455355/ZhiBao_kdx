...
function GetRole() {
    ...
    var id = $.query.get('id');
    var params = { Action: "GetRole", ID: id };
        ...
        success: function (msg) {
            if (msg.errorcode == "0") {
                if (msg.data != null && msg.data[0] != null) {
                    $("#title_txt").val(msg.data[0].RoleName);
                    
                    for (var j = 0; j < msg.data[0].Permissions.length; j++)
                    {
                        var key = msg.data[0].Permissions[j].ModuleKey;
                        var va = msg.data[0].Permissions[j].PermissionsId;
                        $(':input[type="checkbox"][name="' + key + '"][value="' + va + '"]').prop("checked",true);
                    }
...
function UpdateRole() {
    ...
    if (flag) {
        ...
        var params = { Action: "RoleUpdate", ID: $.query.get('id'), Title: $("#title_txt").val(), XXGL: XXGL, ZXGL: ZXGL, CXHDGL: CXHDGL, YHGL: YHGL, FKXXGL: FKXXGL, QXGL: QXGL, CXGL: CXGL, CPDMGL: CPDMGL, TJBBGL: TJBBGL, KHGL: KHGL, CPLXGL: CPLXGL, CPRZGL: CPRZGL, GCCPLXGL: GCCPLXGL, LPGL: LPGL };
        ...
        $.ajax({
            ...
            success: function (msg) {
                if (msg.errorcode == "0") {
                    alert('更新成功！');
                    location.href = "rolelist.aspx?r=" + Math.random();
                } else if (msg.errorcode == "-10") {
                    alert('无权限');
                }
            }
