$(document).ready(function () {
                    ...
                    if (msg.errorcode == "0") {
                        if (msg.data != null && msg.data.length > 0) {
                            for (var i = 0; i < msg.data.length; i++) {
                                ...
                                $('<option data-length="' + msg.data[i].split("|")[2] +'"  data-attr="' + msg.data[i].split("|")[1]+'" value="' + msg.data[i].split("|")[0] + '">' + msg.data[i].split("|")[0] + '</option>').appendTo('#ContentPlaceHolder1_frontProductCode');
                            }
                        }
                    }
                }
    ...
    $('#ContentPlaceHolder1_backcode_ddl').bind('change', function () {
        ...
        if ($('#ContentPlaceHolder1_backcode_ddl').val() != "") {
            var params = { Action: "GetProductCode1", FId: $('#ContentPlaceHolder1_backcode_ddl').val() };
            $.ajax({
                type: "POST",
                url: "../interface/Product.ashx",
                cache: false,
                ...
                success: function (msg) {
                    if (msg.errorcode == "0") {
                        if (msg.data != null && msg.data.length > 0) {
                            for (var i = 0; i < msg.data.length; i++) {
                                var li = msg.data[i].newsid;
                                $('<option data-length="' + msg.data[i].split("|")[2]+'" data-attr="' + msg.data[i].split("|")[1] + '" value="' + msg.data[i].split("|")[0] + '">' + msg.data[i].split("|")[0] + '</option>').appendTo('#ContentPlaceHolder1_backProductCode');
                            }
                        }
                    }
                }
...
$.UserAdd=function()
{
    ...
    $("#divIsQuanCar").removeClass('has-error');
    $("#IsQuanCarerr").html('');


    $("#divLicense").removeClass("has-error");
    $("#errLicense").html("");
    ...
    var isquancar = $("input[type='radio'][name='IsQuanCar']:checked").val()
    ...
    if (typeof (isquancar) != 'undefined') {
        var s_total = $("#ContentPlaceHolder1_frontProductCode").find("option:selected").data("attr");
        var total = $("#ContentPlaceHolder1_frontProductCode").find("option:selected").data("length");
        if (isquancar == 1) {
            if (total == 15 || total == 30) {
                if (s_total < 2.1) {
                    flag = false;
                    $("#divIsQuanCar").addClass('has-error');
                    $("#IsQuanCarerr").html('此卷膜剩余的部分已经不够贴全车了！');
                }
            }
            else {
                $("#divIsQuanCar").addClass('has-error');
                $("#IsQuanCarerr").html('此卷膜不是贴全车膜');
            }
            ...
            if (total == 1.5) {
                if (s_total < 1.5) {
                    flag = false;
                    $("#divIsQuanCar").addClass('has-error');
                    $("#IsQuanCarerr").html('此卷膜剩余的部分已经不够贴单车了！');
                }
            }
            else {
                $("#divIsQuanCar").addClass('has-error');
                $("#IsQuanCarerr").html('此卷膜不是贴单车膜');
            }
        }
        else if (isquancar == 3) {
            if (total == 3.5) {
                if (s_total < 3.5) {
                    flag = false;
                    $("#divIsQuanCar").addClass('has-error');
                    $("#IsQuanCarerr").html('此卷膜剩余的部分已经不够贴单车含侧挡了！');
                }
            }
            else {
                $("#divIsQuanCar").addClass('has-error');
                $("#IsQuanCarerr").html('此卷膜不是贴单车含侧挡膜');
            }
        }
