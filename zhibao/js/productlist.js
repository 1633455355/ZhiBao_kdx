...
if ($("#ContentPlaceHolder1_userstatus").val() == "store") {
    $("#ADealer").hide();
    $("#tbheader").html('<tr><th>操作</th><th>类别</th><th>品牌型号</th><th>状态</th><th>卷轴号</th><th>尺寸(M)</th><th>门店名称</th><th>门店认证时间</th><th>质保年限</th><th>门店是否审核通过</th>');
}
else if ($("#ContentPlaceHolder1_userstatus").val() == "dealer") {
    $("#tbheader").html('<tr><th>操作</th><th>类别</th><th>品牌型号</th><th>状态</th><th>卷轴号</th><th>尺寸(M)</th><th>经销商名称</th><th>经销商认证时间</th><th>门店名称</th><th>门店认证时间</th><th>质保年限</th><th>经销商是否审核通过</th><th>门店是否审核通过</th><th>门店审核操作（<input type="checkbox" name="all_Store" onclick="AllStorecheck();" Id="all_Store"/>全选   <select  id="AudStore" name="AudStore"><option value=""></option><option value="1">认证审核通过</option><option value="2">认证审核未通过</option></select><input type="button"  value="批量审核" onclick="AllAuditStoreProduct()"/>）</th></tr>');
}
else{ 
    $("#tbheader").html('<tr><th>操作</th><th>类别</th><th>品牌型号</th><th>状态</th><th>卷轴号</th><th>尺寸(M)</th><th>经销商名称</th><th>经销商认证时间</th><th>门店名称</th><th>门店认证时间</th><th>质保年限</th><th>是否审核通过</th><th>经销商审核操作（<input type="checkbox" name="all_dealer" onclick="AllDealercheck();" Id="a_dealer"/>全选   <select  id="AudDealer" name="AudDealer"><option value=""></option><option value="1">认证审核通过</option><option value="2">认证审核未通过</option></select><input type="button"  value="批量审核" onclick="AllAuditDealerProduct()"/>）</th><th>门店是否审核通过</th><th>门店审核操作（<input type="checkbox" name="all_Store" onclick="AllStorecheck();" Id="all_Store"/>全选   <select  id="AudStore" name="AudStore"><option value=""></option><option value="1">认证审核通过</option><option value="2">认证审核未通过</option></select><input type="button"  value="批量审核" onclick="AllAuditStoreProduct()"/>）</th></tr>');
}
...
function DoSearch(page_index)
{
    ...
    if (type == "store") {
        $("#tbheader").html('<tr><th>操作</th><th>类别</th><th>品牌型号</th><th>状态</th><th>卷轴号</th><th>尺寸(M)</th><th>门店名称</th><th>门店认证时间</th><th>质保年限</th><th>门店是否审核通过</th></tr>');
    }
    else if (type == "dealer") {
        $("#tbheader").html('<tr><th>操作</th><th>类别</th><th>品牌型号</th><th>状态</th><th>卷轴号</th><th>尺寸(M)</th><th>经销商名称</th><th>经销商认证时间</th><th>门店名称</th><th>门店认证时间</th><th>质保年限</th><th>经销商是否审核通过</th><th>门店是否审核通过</th><th>门店审核操作（<input type="checkbox" name="all_Store" onclick="AllStorecheck();" Id="all_Store"/>全选   <select  id="AudStore" name="AudStore"><option value=""></option><option value="1">认证审核通过</option><option value="2">认证审核未通过</option></select><input type="button"  value="批量审核" onclick="AllAuditStoreProduct()"/>）</th></tr>');
    }
    else {
        $("#tbheader").html('<tr><th>操作</th><th>类别</th><th>品牌型号</th><th>状态</th><th>卷轴号</th><th>尺寸(M)</th><th>经销商名称</th><th>经销商认证时间</th><th>门店名称</th><th>门店认证时间</th><th>质保年限</th><th>经销商是否审核通过</th><th>经销商审核操作（<input type="checkbox" name="all_dealer" onclick="AllDealercheck();" Id="all_dealer"/>全选   <select  id="AudDealer" name="AudDealer"><option value=""></option><option value="1">认证审核通过</option><option value="2">认证审核未通过</option></select><input type="button"  value="批量审核" onclick="AllAuditDealerProduct()"/>）</th><th>门店是否审核通过</th><th>门店审核操作（<input type="checkbox" name="all_Store" onclick="AllStorecheck();" Id="all_Store"/>全选   <select  id="AudStore" name="AudStore"><option value=""></option><option value="1">认证审核通过</option><option value="2">认证审核未通过</option></select><input type="button"  value="批量审核" onclick="AllAuditStoreProduct()"/>）</th></tr>');
    }
    var params = {
                    ...
                    for (var i = 0; i < msg.data.length; i++) {
                        ...
                        if (type == 'managers') {
                            if (msg.data[i].Status == "1" || msg.data[i].Status == "2") {
                                li = li + '<td><input type="checkbox" name="a_dealer"  value="' + msg.data[i].ProductCode.Trim() + '"/><select  id="Dealer_' + msg.data[i].ProductCode.Trim() + '" name="Dealer_' + msg.data[i].ProductCode.Trim() + '">';
                                li = li + '<option value=""></option>'
                                li = li + '<option value="1">认证审核通过</option>';
                                li = li + '<option value="2">认证审核未通过</option>';
                                li = li + '</select><input type="button"  value="审核" onclick="AuditProduct(\'' + msg.data[i].ProductCode.Trim() + '\',\'dealer\')"/></td>';
                            }
                            else {
                                li = li + '<td></td>';
                            }
                        ...
                        if (type == 'managers'|| type == 'dealer') {
                            if (msg.data[i].Status == "2") {
                                li = li + '<td><input type="checkbox" name="a_store"  value="' + msg.data[i].ProductCode.Trim() + '"/><select  id="Store_' + msg.data[i].ProductCode.Trim() + '" name="Store_' + msg.data[i].ProductCode.Trim() + '">';
                                li = li + '<option value=""></option>'
                                li = li + '<option value="1">认证审核通过</option>';
                                li = li + '<option value="2">认证审核未通过</option>';
                                li = li + '</select><input type="button"  value="审核" onclick="AuditProduct(\'' + msg.data[i].ProductCode.Trim() + '\',\'store\')"/></td>';
                            }
                            else {
                                li = li + '<td></td>';
                            }
